...
if ( ! function_exists( 'wpum_profile_card_show_description' ) ) :
/**
 * Display user description in profile-card.php template.
 * 
 * @since 1.0.0
 * @param object $user_data holds WP_User object
 * @access public
 * @return void
 */
function wpum_profile_card_show_description( $user_data ) {

	$description = get_user_meta( $user_data->ID, 'description', true);
	$truncated = wp_trim_words( $description, apply_filters( 'wpum_profile_card_description_length', 20 ) );
	$formatted = wpautop( $truncated );

	echo $formatted;

}
add_action( 'wpum_profile_card_details', 'wpum_profile_card_show_description', 10 );
endif;

if ( ! function_exists( 'wpum_profile_card_show_links' ) ) :
/**
 * Display user links in profile-card.php template.
 * 
 * @since 1.0.0
 * @param object $user_data holds WP_User object
 * @access public
 * @return void
 */
function wpum_profile_card_show_links( $user_data ) {

	$output = '';

	$output .= '<a href="" class="wpum-card-button">'. __('View Profile') .'</a>';
	$output .= '<a href="" class="wpum-card-button">'. __('Send Email') .'</a>';

	echo $output;

}
add_action( 'wpum_profile_card_details', 'wpum_profile_card_show_links', 10 );
endif;