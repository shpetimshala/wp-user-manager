...
/**
 * Display interface for the export/import settings options.
 *
 * @since 1.2.0
 * @return void
 */
function wpum_export_import_settings_display() {

	$url = wp_nonce_url( admin_url( 'users.php?page=wpum-settings&tab=tools' ),'wpum_export_settings', 'wpum_export_nonce' );
	$output = '<a id="wpum-export-settings" href="'.esc_url( $url ).'" class="button">'.esc_html__( 'Export WPUM Settings', 'wpum' ).'</a>';
	echo $output;

}
add_action( 'wpum_export_settings', 'wpum_export_import_settings_display' );
...
function wpum_process_settings_export() {
	...
	if( empty( $_GET['wpum_export_nonce'] ) )
		return;

	if( ! wp_verify_nonce( $_GET['wpum_export_nonce'], 'wpum_export_settings' ) )
		return;
